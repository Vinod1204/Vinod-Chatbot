...
3) Run:
   python multi_turn_chatbot.py --id alice_travel --model gpt-4o-mini --init
   python multi_turn_chatbot.py --id alice_travel
...
class Conversation:
    ...
    participants: Dict[str, Any] = field(default_factory=lambda: {
        "user": {"name": "user"},
    ...
    def add(self, role: str, content: str, *, usage: Optional[Dict[str, int]] = None,
            metadata: Optional[Dict[str, Any]] = None) -> None:
        self.messages.append(Message(
            role=role, content=content, timestamp=utc_now(),
    ...
    def load(self, cid: str) -> Conversation:
        ...
        return Conversation(
            ...
            participants=data.get(
                "participants", {"user": {"name": "user"}, "assistant": {"name": "assistant"}}),
        )
...
class Chatbot:
    ...
    def __init__(self, client: OpenAI, store: ConversationStore,
    ...
    def send(self, cid: str, user_text: str) -> str:
        ...
        if usage_dict is not None:
            ...
            try:
                usage = {k: int(v) for k, v in usage_dict.model_dump(
                ).items() if isinstance(v, (int,))}
            except Exception:
                # fallback
                usage = {"prompt_tokens": getattr(usage_dict, "prompt_tokens", None),
...
def parse_args():
    ...
    ap.add_argument("--root", default="./conversations",
